...
//   mapping (bytes32 => uint256) internal _PartitionwithLock;
  ...
  * @param granularity Granularity of the token.  //granularity粒度
  ...
  */
     ...
     returns (byte, bytes32, bytes32)
  {
    ...
    require(wl.FindPersonal(_to) == true,"need KYC verify");
     ...
     if((_balances[_from] < _value) || (_balanceOfByPartition[_from][_partition] < _value))
